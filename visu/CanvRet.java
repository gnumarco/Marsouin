...
public class CanvRet extends CanGen implements constants.centre,constants.couleur,constants.fourmi,constants.courant,constants.physique, javax.swing.Scrollable {
    ...
    public CanvRet( Memoire m, int moi) {
        mem = m;
        id = moi;
    ...
    public void processImage(){
        ...
        if (myImage==null) System.out.println(java.util.ResourceBundle.getBundle("ressources/canvas").getString("CanvasCarte_:_ERREUR:_myImage=null"));
    ...
    //@@@@@@@@@@@@@@@@@ LES FLECHES @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    /** affiche les fleches qui repr�sentent le courant moyen en chaque point d'estimation de la carte */
    ...
    public void stopAnime() {
        if( threadAnime !=null) {
            threadAnime.arret();
            try{ threadAnime.wait();
            } catch (Exception e){System.out.println("CanvasCarte stopAnime : threadAnim� stopp� "+e);}
            try {
                Thread.yield();
            } catch (Exception e){}
            threadAnime = null;
    ...
    public void processRet(boolean clock){
        degrade = new int[mem.getBatchDataCarte(id).getTailleX()][mem.getBatchDataCarte(id).getTailleY()][3];
        retention = new double[mem.getBatchDataCarte(id).getTailleX()][mem.getBatchDataCarte(id).getTailleY()];
        javax.swing.ProgressMonitor prog = new javax.swing.ProgressMonitor(mem.getFrmVisu(id),"Processing retention...","Processing...",0,0);
        prog.setMaximum((mem.getBatchDataCarte(id).getTailleX()*mem.getBatchDataCarte(id).getTailleY()-mem.getBatchDataCarte(id).getNbSurTerre())*mem.getBatchDataCarte(id).getNbDataCartesTps());
        ProcessRetentionThread prt = new ProcessRetentionThread(mem, degrade, retention, id, prog, clock);
        prt.start();
    }
            ...
            for(int x=0;x<tX;x++){
                for(int y=0;y<tY;y++){
                    if (!mem.getDataCarte(id).getC(x,y).getSurTerre()) {
                        ...
                        gra.setColor(new java.awt.Color(degrade[x][y][0],degrade[x][y][1],degrade[x][y][2]));
